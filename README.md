...
commit-pinned permalink; every quotation is checked word for word; every chapter ends in a
language-agnostic distillation of the pattern. When the code moves, the build fails instead of
quietly lying.
...
excerpts, 994 lines of real source, six interactive diagrams — after that guide had been built,
broken, fixed and verified. The checks in `tools/` are the ones that caught the errors; the rules
in `SKILL.md` are the ones that would have prevented them.
